...
class ChannelInstallTool(Tool):
    ...
    params = {
        ...
        "package": ToolParam(str, "Package name override (default: phb-channel-<name>)", required=False),
    ...
    def execute(
        ...
        pkg = package or f"phb-channel-{channel_name}"
        cmd = ["uv", "tool", "install"]
        if editable:
            cmd.append("--editable")
        cmd.append(pkg)
...
class ChannelSetupTool(Tool):
    ...
    params = {
        "channel_name": ToolParam(str, "Channel name, e.g. 'telegram'"),
        "command": ToolParam(str, "Executable to run for this channel, e.g. 'phb-channel-telegram'"),
