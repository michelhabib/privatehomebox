"""phbgateway CLI entry point.

Thin wrappers around phbgateway.service — all real logic lives there.
...
from phb_commons.process import is_running, read_pid, write_pid
...
def instance_create(
    ...
    console.print(f"\nNext: [bold]phbgateway start --instance {result.instance_name}[/bold]")
...
def _print_instance_status(entry: GatewayInstanceStatusEntry, registry: GatewayRegistry) -> None:
    title = f"phbgateway status — {entry.name}"
    if entry.is_default:
