"""Public service layer for phbgateway instance lifecycle management.

All real business logic lives here.  The CLI in main.py and external callers
(e.g. phbcli tools) both use these functions — no one reaches into main.py
internals directly.
"""
...
from phb_commons.keys import load_public_key_b64 as _load_public_key_b64
...
def _validate_desktop_public_key(key_b64: str) -> None:
    ...
    try:
        ...
        raise GatewayInstanceError(
            ...
            "Provide the base64-encoded Ed25519 public key from 'phbcli status'."
        ) from exc
...
def start_instance(
    ...
    *,
    ...
    cmd = [*uv_python_cmd(), "-m", "phbgateway.main", "start", "--instance", entry.name, "--foreground"]
    if verbose:
        cmd.append("--verbose")
