...
class ChannelPlugin(ABC):
    """Contract between phbcli and a channel plugin.

    Subclass this, implement the abstract methods, then hand an instance to
    ``PluginTransport`` to connect it to phbcli.

    Lifecycle (called by PluginTransport):
      1. ``on_configure(config)`` — push credentials / settings from phbcli
    ...
    Sending / receiving:
      ...
      - Call ``await self.emit(message)`` from within your implementation
        whenever an inbound message arrives; the transport forwards it to phbcli.
    """
    ...
    async def on_configure(self, config: dict[str, Any]) -> None:
        """Receive credentials and settings pushed by phbcli."""
    ...
    async def emit(self, message: UnifiedMessage) -> None:
        """Forward an inbound message from the third party to phbcli.
