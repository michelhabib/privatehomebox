...
The CLI is the primary interface for PHB, but certain tasks are better suited to a persistent visual dashboard:
...
The admin UI runs on a **separate port** from the HTTP API. The existing port allocation scheme assigns 3 ports per workspace slot. This changes to 4:
...
`PORTS_PER_SLOT` increases from `3` to `4`. A new constant `PORT_OFFSET_ADMIN = 3` is added to `phb_commons.constants.network`, and a corresponding `admin_port_for()` helper is added to `workspace.py`.

The `Config` model gains an `admin_port` field with the same default pattern as `http_port` and `plugin_port`.
...
The sidebar drawer uses `behavior="desktop"` so it pushes the page content aside rather than overlaying it. The `bordered` prop adds a separator line between drawer and content area.

```python
ui.left_drawer(value=True).props('behavior="desktop" bordered')
```
...
A table of all registered workspaces with columns: name, status (running/stopped), HTTP port, gateway port, default flag.

Data source: `WorkspaceListTool` for the registry entries; `StatusTool` for the live running state of each workspace.

Actions:
- **Create** — dialog with workspace name and optional path. Calls `WorkspaceCreateTool`. The dialog resets and the table refreshes on success.
- **Remove** — confirmation dialog with an optional **Delete folder from disk** checkbox. Calls `WorkspaceRemoveTool`. The current default workspace cannot be removed until another workspace is set as default.
- **Set default** — star button per row (hidden for the current default). Calls `WorkspaceSetDefaultTool` and refreshes the table immediately.

The table uses `@ui.refreshable` so all three mutation actions call `.refresh()` in-process rather than navigating away.
