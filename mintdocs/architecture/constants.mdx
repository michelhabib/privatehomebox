...
description: "How PHB manages shared constants across packages to avoid duplication and magic values."
icon: "brackets-curly"
---
...
**New workspace services.** If a fourth service is added per workspace slot (e.g. a metrics endpoint), add `PORT_OFFSET_METRICS: int = 3` to `network.py` and bump `PORTS_PER_SLOT` to `4`. No string-searching required — every port calculation already uses the formula.
...
Example for slot 0: `http=18080`, `plugin=18081`, `gateway=18082`.
...
PHB uses the application-reserved range `4000–4999`.
