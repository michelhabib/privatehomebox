...
<Accordion title="flutter pub get reports false SDK incompatibilities (e.g. flex_color_scheme requires Flutter >=3.38.0)">

Running `flutter pub get` (without `fvm`) resolves against the globally installed Flutter SDK, which may be an older version. This produces false incompatibility errors even when the FVM-pinned version satisfies the constraint.

**Fix:** always prefix Flutter commands with `fvm`:
