...
<Steps>
  ...
  <Step title="Enter the code in the mobile app">
    Open the Private Home Box mobile app and enter the code when prompted. The app contacts the gateway, which forwards the pairing request to your running desktop server.
  </Step>
...
<Tip>
  If your mobile app is slow to load, increase the TTL: `phbcli device add --ttl-seconds 600`
</Tip>

<Warning>
  Your desktop server must be running (`phbcli start`) before a pairing request can be processed. The mobile app will not complete pairing if the server is offline.
</Warning>
...
```bash
phbcli device revoke mobile-3f8a2c1d9e4b
